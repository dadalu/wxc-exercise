...
class Solution {
    ...
    public TreeNode dfs(List<Integer> pre, List<Integer> in) {
        ...
        if (!pre.isEmpty()) {
            root = new TreeNode(pre.remove(0));
        } else {
            return root;
        }

        for (int i = 0; i < leftIn.size(); i++) {
            leftPre.add(pre.remove(0));
        }
