...
public class SubdomainVisitCount{
    public static void main(String[] args) {
        ...
        String[] arr = {"9001 discuss.leetcode.com", "9001 leetcode.com", "9001 com"};
    ...
    public List<String> subdomainVisits(String[] cpdomains) {
        ...
        for (int i = 0; i < cpdomains.length; i++) {
            ...
            for (int j = area.length()-1; j >=0; j--) {
                if((area.charAt(j))=='.'){
                    String s = area.substring(j+1);
                    if(map.get(s)==null){
                        map.put(s,Integer.valueOf(count));
                    }else{
                        map.put(s,Integer.valueOf(count)+map.get(s));
                    }
                }
                if(j==0){
                    if(map.get(area)==null){
                        map.put(area,Integer.valueOf(count));
                    }else{
                        map.put(area,Integer.valueOf(count)+map.get(area));
                    }
                }
            }
