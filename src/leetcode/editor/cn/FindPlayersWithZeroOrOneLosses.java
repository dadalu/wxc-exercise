...
class Solution {
    public List<List<Integer>> findWinners(int[][] matches) {
        ...
        for (int i = 0; i < matches.length; i++) {
            int[] match = matches[i];
            int[] win = map.getOrDefault(match[0], new int[]{0, 0});
            int[] lose = map.getOrDefault(match[1], new int[]{0, 0});
            win[0]++;
            lose[1]++;
            map.put(match[0], win);
            map.put(match[1], lose);
        }
        ...
        win.sort(Comparator.comparingInt(a -> a));
        lose.sort(Comparator.comparingInt(a -> a));
